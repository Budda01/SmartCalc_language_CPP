...
int Model::ParseSymbols(const char **str, Model::Stack& string) {
    ...
    if (**str == ' ') {
        ...
        string.check_brt += 1;
        (*str)++;
    } else if (**str == ')' && string.check_brt && string.last_index != OP_BRACKET) {
...
void Model::Transform(Model::Stack& string, std::deque<Model::Node>& queue) {
    ...
    while (!string.stack_data.empty()) {
        ...
        } else if (stack.stack_data.empty() || stack.stack_data.top().type == OP_BRACKET){
            stack.stack_data.push(cur_node);
        } else if (string.stack_data.top().priority > stack.stack_data.top().priority){
            stack.stack_data.push(cur_node);
        }
        string.stack_data.pop();
    }
...
void Model::ProcPriority(Model::Stack& stack, std::deque<Model::Node>& queue, Node cur_node) {
    while (stack.stack_data.top().type != OP_BRACKET && stack.stack_data.top().priority >= cur_node.priority){
        queue.push_back(stack.stack_data.top());
        stack.stack_data.pop();
    }
    stack.stack_data.push(cur_node);
...
void Model::ProcBracket(Model::Stack& stack, std::deque<Model::Node>& queue) {
    while (stack.stack_data.top().type != OP_BRACKET){
        queue.push_back(stack.stack_data.top());
        stack.stack_data.pop();
    }
    stack.stack_data.pop();
...
int Model::ToPostfix(const char *str, std::deque<Model::Node>& queue) {
    ...
    if (strlen(str) != 0 && strlen(str) < 256){
        ...
        // Model::Stack string1 = string;
        // std::cout << "Stringa" << std::endl;
        // while (!string1.stack_data.empty()){
        //     std::cout << string1.stack_data.top().type << std::endl;
        //     string1.stack_data.pop();
        // }
