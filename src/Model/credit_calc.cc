...
int Model::AnnCreditCalc(const std::string& sum, const std::string& time, const std::string& rate, std::string& output) {
    int err = 0;
    long double d_sum = std::stold(sum);
    int d_time = std::stoi(time);
    long double d_rate = std::stold(rate) / 100;
    if (d_time > 0 && d_sum > 0 && d_rate > 0) {
        long double month_pay = (d_sum * d_rate / 12) / (1 - std::pow(1 + d_rate / 12, -d_time));
...
int Model::DiffCreditCalc(const std::string& sum, const std::string& time, const std::string& rate, std::string& output) {
    int err = 0;
    long double d_sum = std::stold(sum);
